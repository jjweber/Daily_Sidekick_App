...
console.log("Data for To Do List: ", toDoData);
...
function renderTodoList() {
  if (!toDoData.todo.length && !toDoData.completed.length) return;

  for (var i = 0; i < toDoData.todo.length; i++) {
    var value = toDoData.todo[i];
    addItemToDOM(value);
  }

  for (var j = 0; j < toDoData.completed.length; j++) {
    var value = toDoData.completed[j];
...
function addItemToDOM(text, completed) {
  console.log("Adding this item to DOM: ", text);
